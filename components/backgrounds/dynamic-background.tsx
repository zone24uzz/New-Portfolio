    ...
    <div className="fixed inset-0 -z-10 overflow-hidden" aria-hidden="true">
      <AnimatePresence mode="crossfade">
        {isDark ? (
          ...
          >
            <div className="absolute inset-0"
              style={{ background: "linear-gradient(160deg, #fffbeb 0%, #fef3c7 50%, #fffbeb 100%)" }} />
            {!isMobile && (
              <ShapeGrid
                direction="diagonal" speed={0.35}
                borderColor="#fde68a" squareSize={44}
                hoverFillColor="#fef3c7" shape="square" hoverTrailAmount={4}
              />
            )}
            <div className="absolute top-1/4 left-1/3 w-[600px] h-[600px] rounded-full pointer-events-none"
            ...
            <div className="absolute bottom-1/3 right-1/4 w-[400px] h-[400px] rounded-full pointer-events-none"
              style={{ background: "radial-gradient(circle, rgba(251,191,36,0.05) 0%, transparent 70%)", filter: "blur(60px)" }} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
